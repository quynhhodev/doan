...
class ProductByCat extends React.Component {
    state = {
        listProduct: [],
        category: [],
        loading: true
    ...
    async componentDidMount() {
        const catSlug = this.props.match.params.slug;
        var res = await axios.get(`/api/product-by-cat/${catSlug}`);
        if (res.data.status === 200) {
            console.log(res);
            this.setState({ listProduct: res.data.product_data.products, category: res.data.product_data.category, loading: false })
        }
        ...
        return (<>
            <div className="py-2 bg-info">
                <div className="container">
                    <h6 className="text-secondary">Danh mục / {this.state.category.catName}</h6>
                </div>
            </div>
            <div className="row">
                {this.state.loading ? "" :
                    listProduct && listProduct.length > 0 && listProduct.map((item, index) => {
                        //var listImage = JSON.parse(item.image)
                        return (
...
export default withRouter(ProductByCat);