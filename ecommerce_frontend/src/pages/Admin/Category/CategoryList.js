...
class CategoryList extends React.Component {
    ...
    async componentDidMount() {
        var res = await axios.get('http://localhost:8000/api/categories');
        if (res.status === 200) {
    ...
    handleDelete = (e, id) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`api/category-to-trash/${id}`).then((res) => {
        ...
        HTML = currentTodos.map((item, index) => {
            return (
                <tr key={index}>
                    ...
                    <td><Link to={`/admin/edit-category/${item.id}`}>Sửa</Link></td>
